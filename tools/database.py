...
class kitti_dataset(Dataset):
    def __init__(self, root, seqs, pos_threshold, neg_threshold) -> None:
        ...
        for seq in seqs:
            pose = np.genfromtxt(os.path.join(root.replace(
                'place_recognition/dataset_overlapnetvlad/ori_dataset', 
                'Dataset/Ori_KITTI/sequences'), seq ,seq + '.txt'))[:, [3, 11]]
    ...
    def load_fea(self, idx, query = True):
        ...
        if query:
            ...
            file = os.path.join(self.root.replace("ori_dataset","snow_dataset"), seq, "BEV_FEA", id + '.npy')
    ...
    def __getitem__(self, idx):
        ...
        query_fea = self.load_fea(queryid,query = True)
        pos_fea = self.load_fea(posid, query = False)
        neg_fea = self.load_fea(negid, query = False)
        other_fea = self.load_fea(otherid, query = False)
